...
XsLilStatus xs_lil_parse_array_record(XsLilModule *module, const char *text, size_t length, XsLilError *error)
{
  ...
  if(id_end == cursor || id > UINT32_MAX || id != xs_lil_module_array_type_count(module) ||
     end - id_end < 4 || strncmp(id_end, " : ", 3) != 0)
  ...
  if(separator == nullptr || separator == cursor || separator + 3 >= end)
    return xs_lil_set_error(error, XS_LIL_INVALID_ARGUMENT, "XLIL array type record is invalid");
  XsLilType element_type = {0};
  if(!xs_lil_parse_type_name(module, cursor, (size_t)(separator - cursor), &element_type))
