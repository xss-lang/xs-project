                                         ...
                                         XsLilType *type, XsLilError *error)
{
  ...
  if(module == nullptr || type == nullptr || length == 0 || element_type.kind == XS_LIL_TYPE_VOID ||
  ...
  for(size_t index = 0; index < module->array_type_count; ++index)
    if(xs_lil_type_equal(module->array_types[index].element_type, element_type) &&
       module->array_types[index].length == length)
  ...
  module->array_types[module->array_type_count++] = (XsLilArrayType){.element_type = element_type, .length = length};
