...
static bool symbol_visible_from(const XsHirSymbol *symbol, const char *current_namespace)
{
  return symbol != NULL &&
         (strcmp(symbol->namespace_name, current_namespace) == 0 || symbol->visibility == XS_SYNTAX_VISIBILITY_PUBLIC);
}
                                 ...
                                 XsDiagnostics *diagnostics)
{
  ...
  if (!symbol_visible_from(symbol, namespace_name)) {
    char message[512];
                                       ...
                                       const XsHirImportScope *imports, XsDiagnostics *diagnostics)
{
  ...
  if (node->kind == XS_SYNTAX_EXPR_CALL && node->child_count != 0)
    success = validate_call_target(node->children[0], function, namespace_name, project_symbols, imports, diagnostics);
  else if (node->kind == XS_SYNTAX_EXPR_METHOD_CALL)
    success = validate_call_target(node, function, namespace_name, project_symbols, imports, diagnostics);
  for (size_t i = 0; i < node->child_count; ++i)
    success = validate_name_uses_in_node(node->children[i], function, namespace_name, project_symbols, imports,
                                         diagnostics) &&
              success;
  return success;
}
                               ...
                               const XsHirImportScope *imports, XsDiagnostics *diagnostics)
{
  ...
  for (size_t i = 0; i < project_symbols->count; ++i) {
    ...
    if (symbol->kind != XS_HIR_SYMBOL_FUNCTION || symbol->span.file_id != tree->file_id)
      continue;
    success = validate_name_uses_in_node(symbol->syntax, symbol->syntax, symbol->namespace_name, project_symbols,
                                         imports, diagnostics) &&
              success;
  }
