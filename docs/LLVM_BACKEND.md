...
- Initial XLIL body lowering for `const i64`, `br`, `ret`, and `ret %rN`
...
`ret %rN` in parameterless functions. The backend emits declarations from the public C API and direct `.xlil` files after
