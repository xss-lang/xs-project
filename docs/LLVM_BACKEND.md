...
- Initial XLIL body lowering for parameters, constants, i32 arithmetic/bitwise/shift/comparison, i64
  arithmetic/bitwise/shift/comparison, `call`, `br`, `br_if`, `panic`, `ret`, and `ret %rN`
...
and `ret %rN`. `panic` emits an `llvm.trap` call followed by LLVM `unreachable`. Parameter values are read from the declared
