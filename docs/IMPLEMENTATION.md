...
- Plain source native builds support the first expression slice for top-level `fn main() => Long`: i32-range integer
  literals, `+`, `-`, `*`, `/`, `%`, and one top-level `if` expression with a bool literal or i32 comparison condition.
  `!=` is represented by lowering equality and swapping branch targets. This lowers through C MIR, XLIL, LLVM IR, object
  emission, and native `.xse` linking.
...
- `xs build -file <input.xs>` and `xs build -proj <input.xsproj>` can now use the same native path for the first checked
  ...
  integer literals and `+`, `-`, `*`, `/`, or `%`. This source bridge creates a temporary C MIR function, lowers it to XLIL,
  and then reuses the XLIL native builder.
