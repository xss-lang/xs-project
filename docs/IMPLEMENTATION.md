...
- MIR functions carry an explicit XLIL-vocabulary return type. The first MIR → XLIL body bridge lowers typed MIR
  `const.i64` local statements and matching local return values to XLIL `const` instructions and `ret %N` terminators.
