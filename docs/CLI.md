...
`xs --version` prints the compiler version, such as `xs 0.0.4`.
...
one return statement. The supported return expression subset is i32-range integer literals, unary `-`, `+`, `-`, `*`, `/`,
`%`, and one top-level `if (...) { expr; } else { expr; }` expression whose condition is a bool literal, unary `!`, or an
i32 comparison, including `==`, `!=`, `<`, `<=`, `>`, and `>=`. The compiler lowers that
